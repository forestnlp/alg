package com.alg.leetcode.graph;
...
public class P743M {
    ...
    class Solution {
        ...
        public int networkDelayTime(int[][] times, int N, int K) {
            ...
            //construct
            for (int[] edge : times) {
                List<Node> adj = map.getOrDefault(edge[0], new ArrayList<>());
                adj.add(new Node(edge[1], edge[2]));
                map.put(edge[0], adj);
            }
            ...
            //add seed
            PriorityQueue<Node> pq = new PriorityQueue<>((o1, o2) -> distance[o1.id]-distance[o2.id]);
            ...
            while (!pq.isEmpty()) {
                ...
                List<Node> neighbors = map.getOrDefault(node.id, Collections.emptyList());
                for (Node neighbor : neighbors) {
                    distance[neighbor.id] = Math.min(distance[neighbor.id], distance[node.id] + neighbor.distance);
                    pq.offer(neighbor);
                }
            }
